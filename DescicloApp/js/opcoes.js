function salvar() {
	...
	if (alternativo != "outro") {
		localStorage["alternativo_favorito"] = alternativo;
	} else {
		localStorage["alternativo_favorito"] = document.getElementById("dominio").value;
		localStorage["napopup"] = document.getElementById("napopup").value;
	};
	//apenas pro usuario entender o que e o na popup
	if (alternativo == "desciclopedia.org") {
		localStorage["napopup"] = ".org"
	};
	if (alternativo == "desciclopedia.ws") {
		localStorage["napopup"] = ".ws"
	};
	if (alternativo == "pudim.info") {
		localStorage["napopup"] = ".info"
	};
	if (alternativo == "vist.as") {
		localStorage["napopup"] = ".as"
	};
	if (alternativo == "pesquisa.la") {
		localStorage["napopup"] = ".la"
	};
	if (alternativo == "desnoticias.org") {
		localStorage["napopup"] = "desnoticias.org"
	};
	if (alternativo == "sophia.desciclopedia.org") {
		localStorage["napopup"] = "sophia."
	};
	if (alternativo == "roberto.desciclopedia.org") {
		localStorage["napopup"] = "roberto."
	};
	...
	if (document.getElementById("coricone").checked == true) {
		localStorage["coricone"] = document.getElementById("coricone").value;
		...
		localStorage["cor2"] = "#000000";
	} else {
		localStorage["coricone"] = document.getElementById("coricone2").value;
		localStorage["iricone"] = "imagens/avancarwp7b.png";
		localStorage["editaricone"] = "imagens/novowp7b.png";
		localStorage["pesquisaricone"] = "imagens/buscawp7b.png";
		localStorage["cor2"] = "#ffffff";
	};
	if (username.value != "") {
		localStorage["hifen"] = '&nbsp;-&nbsp';
	} else {
		localStorage["hifen"] = ''
	};
	...
	if (document.getElementById("username").value != localStorage["username"]) {
		bg.removePage('http://pesquisa.la/index.php?title=Usu%C3%A1rio_Discuss%C3%A3o:' + localStorage.getItem('username') + '&action=history&printable=yes', bg.updateBadge);
	}
	localStorage["username"] = document.getElementById("username").value;
	if (document.getElementById("notifmsg").checked == true) {
		if (username.value == "") {
			if (confirm('Para ativar as notifica\u00e7\u00F5es, escreva seu username primeiro')) {
				document.getElementById("notifmsg").checked = false;
			} else {
				document.getElementById("notifmsg").checked = false;
			}
		}
	...
	if (document.getElementById("desciclotoolbar").checked == true) {
		localStorage["pref_tb_is_visible"] = true;
	} else {
		localStorage["pref_tb_is_visible"] = false;
	};
	if (document.getElementById("botaofeedback").checked == true) {
		localStorage["botaofeedback"] = true;
	} else {
		localStorage["botaofeedback"] = false;
		if (!localStorage.feedback) {
			new Messi('<iframe id="JotFormIFrame" allowtransparency="true" src="http://aloogle.tumblr.com/descicloapp/feedbackbotaodesativado" frameborder="0" style="width:100%; height:550px; border:none;" scrolling="no"></iframe>', {
				title : 'Feedback'
			});
...
if (!localStorage.storagepadrao) {
	...
	localStorage["notifications_enabled"] = "false";
...
function redefinir() {
	...
	bg.location.reload()
	chrome.notifications.create("redef", {
		type : "basic",
		iconUrl : "../icons/icon_64.png",
		title : "DescicloApp",
		message : "Tem certeza que quer redefinir as configura\u00e7\u00F5es?",
		buttons : [{
				title : 'Sim',
				iconUrl : '../imagens/checkmarkicon.png'
			}, {
				title : 'N\u00E3o',
				iconUrl : '../imagens/x.png'
			}
		],
		priority : 0
	}, function () {});
	chrome.notifications.onButtonClicked.addListener(function (redef, buttonIndex) {
		if (buttonIndex == 0) {
			...
			if (!localStorage.storagepadrao) {
				...
				localStorage["pref_tb_is_visible"] = "false";
				localStorage["check_interval"] = "5400";
				localStorage["notifications_enabled"] = "true";
				localStorage.storagepadrao = "true";
			}
...
$(function () {
	...
	if (localStorage.getItem('coricone') != "imagens/buscawp72b.png") {
		document.getElementById("coricone").checked = true;
	} else {
		document.getElementById("coricone2").checked = true;
	};
	if (localStorage.getItem('buscacontexto') != "false") {
		document.getElementById("buscacontextosim").checked = true;
	} else {
		document.getElementById("buscacontextonao").checked = true;
	};
	if (localStorage.getItem('descicloguia') != "false") {
		document.getElementById("descicloguiasim").checked = true;
	} else {
		document.getElementById("descicloguianao").checked = true;
	};
	if (localStorage.getItem('pref_tb_is_visible') != "true") {
		document.getElementById("desciclotoolbar").checked = false;
		document.getElementById("botaomais").disabled = true;
		document.getElementById("barrabusca").disabled = true;
		document.getElementById("botaofeedback").disabled = true;
	} else {
		document.getElementById("desciclotoolbar").checked = true;
	};
	if (localStorage.getItem('notifications_enabled') != "true") {
		document.getElementById("notifmsg").checked = false;
	} else {
		document.getElementById("notifmsg").checked = true;
	};
	if (localStorage.getItem('botaomais') != "true") {
		document.getElementById("botaomais").checked = false;
	} else {
		document.getElementById("botaomais").checked = true;
	};
	if (localStorage.getItem('barrabusca') != "true") {
		document.getElementById("barrabusca").checked = false;
	} else {
		document.getElementById("barrabusca").checked = true;
	};
	if (localStorage.getItem('botaofeedback') != "true") {
		document.getElementById("botaofeedback").checked = false;
	} else {
		document.getElementById("botaofeedback").checked = true;
	};
	if (localStorage.getItem('barrabuscapopup') != "true") {
		document.getElementById("barrabuscapopup").checked = false;
	} else {
		document.getElementById("barrabuscapopup").checked = true;
	};
	...
	$('#escolhapopup').click(function () {
		new Messi('<iframe src="botoespopup.html" frameborder="0" style="width:100%; height:515px; border:none;" scrolling="no"></iframe>', {
			buttons : [{
					id : 0,
					label : 'Fechar',
					val : 'X'
	...
	$('#escolha').click(function () {
		new Messi('<iframe src="botoes.html" frameborder="0" style="width:100%; height:515px; border:none;" scrolling="no"></iframe>', {
			buttons : [{
					id : 0,
					label : 'Fechar',
					val : 'X'
	...
	$('#desciclotoolbar').click(function () {
		if (document.getElementById("desciclotoolbar").checked == false) {
			document.getElementById("botaomais").disabled = true;
			document.getElementById("barrabusca").disabled = true;
			document.getElementById("botaofeedback").disabled = true;
		} else {
			document.getElementById("botaomais").disabled = false;
			document.getElementById("barrabusca").disabled = false;
			document.getElementById("botaofeedback").disabled = false;
		};
	})
	$('#descicloapp').click(function () {
		descicloApp()
	});
