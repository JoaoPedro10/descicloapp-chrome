...
}, BG = chrome.extension.getBackgroundPage(), DB = openDatabase("pages", "1.0", "Monitored Pages", 51380224),
REGEX_TIMEOUT = 7E3, REGEX_WORKER_PATH = "scripts/regex.js", REQUEST_TIMEOUT = 1E4, MIN_BODY_TAIL_LENGTH = 100, DATABASE_STRUCTURE = "CREATE TABLE IF NOT EXISTS pages (   `url` TEXT NOT NULL UNIQUE,   `name` TEXT NOT NULL,   `mode` TEXT NOT NULL DEFAULT 'text',   `regex` TEXT,   `selector` TEXT,   `check_interval` INTEGER,   `html` TEXT NOT NULL DEFAULT '',   `crc` INTEGER NOT NULL DEFAULT 0,   `updated` INTEGER,   `last_check` INTEGER,   `last_changed` INTEGER );";
(function () {
...
function describeTime(a) {
	...
	e = "";
	if (c)
		var f = chrome.i18n.getMessage("day"), g = chrome.i18n.getMessage("days", c.toString()), e = e + (1 == c ? f : g);
...
function sqlResultToArray(a) {
	for (var b = [], d = 0; d < a.rows.length; d++)
		b.push(a.rows.item(d));
	return b
}
...
function setPageSettings(a, b, d) {
	...
	f;
	for (f in b)
		c.push(f + " = ?"), "boolean" == typeof b[f] && (b[f] = new Number(b[f])), e.push(b[f]);
	e.push(a);
	c ? (a = "UPDATE pages SET " + c.join(", ") + " WHERE url = ?", executeSql(a, e, null, d)) : (d || $.noop)()
}
function addPage(a, b) {
	if (window != BG)
		return BG.addPage(a, b);
	executeSql("INSERT INTO pages(url, name, mode, regex, selector,                                  check_interval, html, crc, updated,                                  last_check, last_changed)                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [a.url, a.name || chrome.i18n.getMessage("untitled", a.url), a.mode || "text", a.regex || null, a.selector || null, a.check_interval || null, a.html || "", a.crc || 0, a.updated ? 1 : 0, Date.now(), a.last_changed || null], null,
		function () {
		BG.takeSnapshot();
		BG.scheduleCheck();
		(b || $.noop)()
