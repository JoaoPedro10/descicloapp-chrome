 ...
 * @param {Object} states An object whose keys are state names and whose values are objects whose keys are event names and whose values are transition functions that return a new state
 ...
 * @param {Function} unexpectedEvent An optional function that is called when an event is received that is unexpected when the machine is in the current state
 * @param {Function} unknownState An optional function that is called when a transition function returns an unknown state.
 *
 */
function StateMachine(name, initial, states, unexpectedEvent, unknownState) {
    ...
    // XXX ajvincent Should we validate that we have enough states?
    ...
    var self = this;
    var current = initial;
    //console.log("%s: state machine starting up in state %s", name, current);
    ...
    this.handleEvent = function (event) {
        ...
        // XXX ajvincent What should happen if an error is thrown?
        var next = (states[current][event] || logUnexpectedEvent).apply(this, arguments);
        //console.log("%s: state machine transitioning to new state %s after event %s", name, next, event);
        if (!(next && states[next]))
...
/**
 * Converts an ordinary function into a message receiver.
 *
